...
serve(async (req) => {
  ...
  try {
    ...
    // Use Google Gemini API directly with image generation model
    ...
    console.log('AI response received:', { hasCandidates: !!aiData.candidates?.[0] });

    const inlineData = aiData.candidates?.[0]?.content?.parts?.[0]?.inlineData;
    if (!inlineData || !inlineData.data) {
      console.error('No image in response:', JSON.stringify(aiData, null, 2));
      throw new Error('No image generated by AI');
    }

    // Extract base64 data directly from inlineData
